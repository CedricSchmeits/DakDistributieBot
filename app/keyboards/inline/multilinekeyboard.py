...
from app.utils.markupconstructor import MarkupConstructor


class MultilineKeyboard(MarkupConstructor):
    """
    ...
    """
    aliases = {
        'callback_data': ('cb', 'cd', 'callback', 'data')
    }
    ...
    def _replace_aliases(
            self,
            action: Dict[str, Union[str, bool, Tuple[Dict[str, str], CallbackData], LoginUrl, CallbackGame]]
    ):
    ...
    def _set_callback_data(
            button_data: Dict[str, Union[str, bool, Tuple[Dict[str, str], CallbackData], LoginUrl, CallbackGame]],
    ):
        if isinstance(button_data['callback_data'], tuple):
            button_data['callback_data'] = button_data['callback_data'][0].new(button_data['callback_data'][1:])
        ...
        for action in actions:
            ...
            if 'pay' in button_data:
                if len(buttons) != 0 and button_data['pay']:
                    raise ValueError('pay type of button must always be the first button in the first row')
