...
# Nieuwe Edition
##########################################################
class NewEdition(StatesGroup):
    Naam = State()
    Url = State()
    Cover = State()
    Confirm = State()

@dp.message_handler(Text(equals='annuleren', ignore_case=True), state=NewEdition)
async def AdminEditionCancel(message: types.Message, state: FSMContext):
...
async def AdminEditionAdd(message: types.Message):
    ...
    await NewEdition.first()


@dp.message_handler(state=NewEdition.Naam, is_superuser=True)
async def AdminEditionSetName(message: types.Message, state: FSMContext):
    ...
    await NewEdition.next()
    await state.update_data(item=item)


@dp.message_handler(state=NewEdition.Url, is_superuser=True)
async def AdminEditionSetUrl(message: types.Message, state: FSMContext):
    if not message.text:
...
@dp.message_handler(state=NewEdition.Cover, is_superuser=True)
async def AdminEditionSetCover(message: types.Message, state: FSMContext):
    if not message.text:
...
@dp.callback_query_handler(is_superuser=True, state=NewEdition.Confirm, text='confirm_item')
async def AdminEditionConfirm(query: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    item = data.get('item')
...
@dp.callback_query_handler(is_superuser=True, state=NewEdition.Confirm, text='reset_item')
async def AdminEditionReset(query: types.CallbackQuery, state: FSMContext):
...
async def AdminChangesEdition(query: types.CallbackQuery, state: FSMContext):
